 ...
 * @param alignments
 */
function restoreOpacityForAlignments(svg_canvas, alignments) {
    for (const align of Object.values(alignments)) {
        const restoreOpacityVal = 1;
        svg_canvas.selectAll('#a'+ align.id).style('opacity', restoreOpacityVal);
        svg_canvas.select('#gTree1').selectAll('#n' + align.e1.id).style('opacity', restoreOpacityVal);
        svg_canvas.select('#gTree2').selectAll('#n' + align.e2.id).style('opacity', restoreOpacityVal);
...
function unhighlightNode(svg_canvas, node) {
 ...
 * @param {*} svg_canvas svg:g of baseline or matrix
 * @param {*} alignmentSet base_alignment or mtrx_alignment
 */
function highlightAlignment(alignments, svg_canvas, alignmentSet, adaptation) {
    ...
    for (let almt of allAlignments.reverse()) {
        ...
        svg_canvas.select("#gMap").select('#a'+almt.id)
            .style('opacity', 1)
            // .classed('muted', false)
            .raise();

        svg_canvas.select("#gMap").select('#a'+almt.id).select('.mapLine-fg').style('stroke', '#0077ff').style('stroke-width', adaptiveMaplineFgStrokeWidth+'px');
        svg_canvas.select("#gMap").select('#a'+almt.id).select('.mapLine-bg ').style('stroke-width', adaptiveMaplineBgStrokeWidth+'px');
        svg_canvas.select("#gMap").select('#a'+almt.id+'.map-to-hidden').select('path.mapLine-fg').style('stroke-dasharray', '2 6').style('stroke-width', adaptiveHiddenMaplingFgStrokeWidth+'px');

        //tree nodes .node .branch|.leaf (all nodes in the currently cliked mapping can be highlighted
        svg_canvas.select("#gTree1").select('#n'+almt.e1.id)
            // .classed('muted', false)
            .style('opacity', 1)
            .select('text')
            .style('font-weight', adaptiveFontWeight);
        ...
        svg_canvas.select("#gTree2").select('#n'+almt.e2.id)
            // .classed('muted', false)
            .style('opacity', 1)
            .select('text')
            .style('font-weight', adaptiveFontWeight);
        ...
        while (leftAlignParent != null) {
            const parentNode = svg_canvas.select('#gTree1').select('#n'+leftAlignParent.id)
        ...
        while (rightAlignParent != null) {
            const parentNode = svg_canvas.select('#gTree2').select('#n'+rightAlignParent.id)
            if (!$(parentNode.node()).hasClass('expanded'))
                parentNode.style('opacity', 1).style('font-weight', adaptiveFontWeight)
    ...
    svg_canvas.selectAll('.mapping').filter(d => d.overlappedTop).raise();
    allAlignments.forEach(almt => {
        if(almt.overlappedTop)
            svg_canvas.select('#gMap').select('#a'+almt.id).raise();
    });
...
function unhighlightAll(svg_canvas) {
    //Reset foreground and background map lines to their default stroke-width
    svg_canvas.selectAll('.mapLine-fg').style('stroke-width', '1px');
    svg_canvas.selectAll('.mapLine-bg').style('stroke-width', '4.5px');

    //Reset the dotted maplines
    svg_canvas.selectAll('.map-to-hidden.mapLine-fg').style('stroke-dasharray', '2 4')

    //Restore the font-weight of the nodes (ontology class)
    svg_canvas.selectAll('.node>text').style('font-weight', 100)

    //Always place direct mappings on top.
    svg_canvas.selectAll('.map-to-hidden').lower();

    //Reset all text in general as a safety precaution
    svg_canvas.selectAll('text').style('font-weight', 100)
}

function restoreToBaselineAdaptations(svg_canvas) {
    restoreOpacity(svg_canvas);
    unhighlightAll(svg_canvas);
}